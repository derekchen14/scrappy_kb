...
# Get Auth0 config from centralized settings
AUTH0_DOMAIN = settings.auth.DOMAIN
AUTH0_AUDIENCE = settings.auth.AUDIENCE
AUTH0_ALGORITHMS = settings.auth.ALGORITHMS
...
def require_auth(f):
    """
    Decorator to require authentication for a route (currently not used)
    """
    @wraps(f)
